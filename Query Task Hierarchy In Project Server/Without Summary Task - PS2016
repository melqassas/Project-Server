WITH CTE(ProjectUID, TaskUID, TaskParentUID, TaskName, LEVEL) AS
  (SELECT ProjectUID,
          ...
          0
   FROM pjrep.MSP_EpmTask_UserView WITH(NOLOCK)
   ...
   UNION ALL SELECT UV.ProjectUID,
                    ...
                    CTE.Level + 1
   FROM CTE
   INNER JOIN pjrep.MSP_EpmTask_UserView UV WITH(NOLOCK) ON CTE.ProjectUID = UV.ProjectUID
   AND CTE.TaskParentUID = UV.TaskUID
   WHERE TaskIsSummary = 1
     AND CTE.TaskParentUID <> UV.TaskParentUID )
SELECT X.TaskUID,
       X.TaskName AS 'Task Hierarchy'
FROM CTE X
INNER JOIN
  (SELECT ProjectUID,
          TaskUID,
